...
func accounts(c *cli.Context) error {
	...
	for _, sPort := range ports {
		...
		err = client.Call(&accounts, "cph_accounts")
		if err != nil {
			fmt.Println("client.call:cph_accounts err", err)
			return err
		}
		for k, v := range accounts {
			if v != accounts[1] {
				...
				err = client.Call(&result, "cph_sendTransaction", args)
				if err != nil {
					fmt.Println("client.call:cph_sendTransaction err", err)
					return err
				}
			}
			err = client.Call(&result, "cph_blockNumber")
			if err != nil {
				fmt.Println("client.call:cph_txBlockNumber err", err)
				return err
			}

			err = client.Call(&result, "cph_getBalance", v, string(result))
			if err != nil {
				fmt.Println("client.call:cph_getBalance err", err, "addr", v)
				return err
			}
...
func auto_miner_start(c *cli.Context) error {
	...
	for _, sPort := range ports {
		...
		err = client.Call(&account, "cph_accounts")
		if err != nil {
			fmt.Println("client.call:cph_accounts err", err)
			return err
		}
...
func auto_trans(c *cli.Context) error {
	...
	*/
	...
	err = client.Call(&result, "cph_autoTransaction", 1, nTime)
	if err != nil {
		fmt.Println("client.call: cph_autoTransaction err", err)
		return err
	}
	fmt.Println(sPort+" cph_autoTransaction OK!", result)
	return nil
...
func auto_stop(c *cli.Context) error {
	...
	if sPort != "" {
		...
		//var account[]string
		var result string
		err = client.Call(&result, "cph_autoTransaction", 0, 5)
		if err != nil {
			fmt.Println("client.call: cph_autoTransaction(0,5) err", err)
			return err
		}
		fmt.Println(sPort+" cph_autoTransaction(0,5) stop OK! ", result)
	}
