...
append_node()
{
    ...
    for m in $( seq $NODES_NUM ); do
        ...
        if [[ "$n" -eq 1 ]]; then
            $CYPHER  --nogas --onetdebug $ONETLOGLEVEL --onetport $((7100 + 2 * $n)) --nat=extip:$LOCALIP --ws  --ipencdis $IPENCDISVALUE --localtest $LOCALIP  -wsaddr="0.0.0.0" --wsorigins "*" --tps --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi cph,web3,personal,miner --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity "$LOGLEVEL" --datadir "$NODE_DIR/$n" --networkid $NetWorkId --gcmode archive --bootnodes "$BOOTNODE" 2>"$NODE_DIR/$n/$n.log" &
        else
            $CYPHER  --nogas --onetdebug $ONETLOGLEVEL --onetport $((7100 + 2 * $n)) --nat=extip:$LOCALIP --tps --ipencdis $IPENCDISVALUE --localtest $LOCALIP  --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi cph,web3,personal,miner --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity "$LOGLEVEL" --datadir "$NODE_DIR/$n" --networkid $NetWorkId --gcmode archive --bootnodes "$BOOTNODE" 2>"$NODE_DIR/$n/$n.log" &
        fi
    done
...
console_node()
{
    ...
    log "console $NODES_NUM node...."
        n=$(($1 + $NODES_NUM))
        $CYPHER  --nogas --onetdebug $ONETLOGLEVEL --onetport $((7100 + 2 * $n)) --nat "none" --tps --ipencdis $IPENCDISVALUE --localtest $LOCALIP --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi cph,web3,personal,miner --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity "$LOGLEVEL" --datadir "$NODE_DIR/$NODES_NUM" --networkid $NetWorkId --gcmode archive --bootnodes "$BOOTNODE" console
...
attach_node()
{
       ...
       $CYPHER  --nogas --onetdebug $ONETLOGLEVEL --onetport $((7100 + 2 * $n)) --nat "none" --tps --ipencdis $IPENCDISVALUE --localtest $LOCALIP --rpc --rpccorsdomain "*" --rpcaddr 0.0.0.0 --rpcapi cph,web3,personal,miner --port $((16000 + 2 * $n)) --rpcport $((18000 + 2 * $n)) --verbosity "$LOGLEVEL" --datadir "$NODE_DIR/$NODES_NUM" --networkid $NetWorkId --gcmode archive --bootnodes "$BOOTNODE"  2>"$NODE_DIR/$NODES_NUM/$NODES_NUM.log" &
