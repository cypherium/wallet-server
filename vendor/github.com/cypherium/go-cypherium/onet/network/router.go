...
func NewRouter(own *ServerIdentity, h Host) *Router {
	...
	r.address = h.Address()
	log.Lvlf2("New router with address %s and public key %s", r.address, r.ServerIdentity.Public)
	return r
}
...
func (r *Router) Send(e *ServerIdentity, msg Message, bForeConnect bool) (uint64, error) {
	...
	if e.Address.String() == r.ServerIdentity.Address.String() {
...
func (r *Router) closeConnect(id ServerIdentityID, c Conn) {
	if err := c.Close(); err != nil {
		log.Lvl5(r.address, "having error closing conn to", c.Remote(), ":", err)
	}
	rx, tx := c.Rx(), c.Tx()
	...
	log.Lvl4("@closeConnect", c.Remote(), "rx", rx, "tx", tx)	
}
