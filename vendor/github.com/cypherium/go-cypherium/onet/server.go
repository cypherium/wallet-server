...
func NewTcpServer(addr string) *Server {
	...
	serverIdentity.Address = network.Address("tcp://" + addr)
	return NewServerTCPWithListenAddr(serverIdentity, network.EncSuite, "")
}
...
func (c *Server) Start() {
	...
	InformServerStarted()
	c.started = time.Now()
	log.Lvlf2("Starting server at %s on address %s with public key %s",
