...
import (
	...
	"github.com/cypherium/go-cypherium/onet/cfgpath"
...
func InteractiveConfig(suite network.Suite, binaryName string) {
	...
	serverBinding = network.NewAddress(network.PlainTCP, net.JoinHostPort(hostStr, portStr))
	...
	if !ipProvided {
		...
		if err != nil {
			...
			if err != nil {
				...
				publicAddress = network.NewAddress(network.PlainTCP, strings.TrimSpace(string(buff))+":"+portStr)
			}
		}
	} else {
...
func askReachableAddress(port string) network.Address {
	...
	return network.NewAddress(network.PlainTCP, ipStr)
}
