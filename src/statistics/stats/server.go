...
func (srv *Server) APIHandler(w http.ResponseWriter, r *http.Request) {
	...
	node, ok := srv.Get(model.ID(nodeID))
	if !ok {
		http.NotFound(w, r)
		return
	}
...
func (srv *Server) Join(conn net.Conn) error {
	...
	for {
		...
		case "hello":
			...
			defer func() {
				srv.Collect(model.DisconnectReport{report.NodeID()})
			}()
		...
		case "latency":
			report := model.LatencyReport{}
			if err = json.Unmarshal(emit.Payload, &report); err != nil {
				break
			}
			err = srv.Collect(report)
		case "block":
			report := model.BlockReport{}
			if err = json.Unmarshal(emit.Payload, &report); err != nil {
				break
			}
			err = srv.Collect(report)
		case "pending":
			report := model.PendingReport{}
			if err = json.Unmarshal(emit.Payload, &report); err != nil {
				break
			}
			err = srv.Collect(report)
